...
// QueryIperfTests chains the current query on the "iperf_tests" edge.
func (hq *HostQuery) QueryIperfTests() *IperfTestQuery {
	query := (&IperfTestClient{config: hq.config}).Query()
...
func (hq *HostQuery) Clone() *HostQuery {
	...
	return &HostQuery{
		...
		withIperfTests: hq.withIperfTests.Clone(),
...
// WithIperfTests tells the query-builder to eager-load the nodes that are connected to
// the "iperf_tests" edge. The optional arguments are used to configure the query builder of the edge.
func (hq *HostQuery) WithIperfTests(opts ...func(*IperfTestQuery)) *HostQuery {
	query := (&IperfTestClient{config: hq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	hq.withIperfTests = query
	return hq
}
...
func (hq *HostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Host, error) {
	...
	if query := hq.withIperfTests; query != nil {
		if err := hq.loadIperfTests(ctx, query, nodes,
			func(n *Host) { n.Edges.IperfTests = []*IperfTest{} },
			func(n *Host, e *IperfTest) { n.Edges.IperfTests = append(n.Edges.IperfTests, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (hq *HostQuery) loadIperfTests(ctx context.Context, query *IperfTestQuery, nodes []*Host, init func(*Host), assign func(*Host, *IperfTest)) error {
	...
	query.withFKs = true
	query.Where(predicate.IperfTest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(host.IperfTestsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.host_iperf_tests
		if fk == nil {
			return fmt.Errorf(`foreign-key "host_iperf_tests" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "host_iperf_tests" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
